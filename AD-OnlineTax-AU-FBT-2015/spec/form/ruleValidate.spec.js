...
function ruleValidation(ruleData, ruleName) {
  describe(ruleName + "Validations", function () {
    ...
    csv.parse(ruleData, {columns: true}, function (err, allRows) {
      ...
      dataRows.forEach(function (row, index) {
        ...
        it("AU FBT 2015 " + ruleName + "Validation Row " + (index + 5) + ": " + row["MYOB Name"], function () {
          var inputFields = {};
          ...
          if (index + 2 <= dataRows.length && !dataRows[index + 1]["MYOB Name"] && !dataRows[index + 2]["MYOB Name"]) {
            ...
            var actualErrorFields =  Object.keys(actualOutputFields).filter(function(fieldId) {
              if (actualOutputFields[fieldId].errors && !expectedErrorCodes[fieldId]) {
                return fieldId;
              }
            });
            ...
            allErrorFields.forEach(function (fieldId) {
              // Ignore if no value found
              if (!actualOutputFields[fieldId]) {
                return;
              }
              if (!expectedErrorCodes[fieldId] && !actualOutputFields[fieldId].errors) {
                return;
              }
              else {
                ...
                expect(actualOutputFields[fieldId].errors, "Missing actual errors for " + fieldId).to.exist;
                expect(expectedErrorCodes[fieldId], "Missing expected errors for " + fieldId).to.exist;
                expect(actualOutputFields[fieldId].errors, "Missing errors for " + fieldId).to.have.length(expectedErrorLength);
                var actualErrorCode = [];
                var actualMessageCode = [];
                actualOutputFields[fieldId].errors.forEach(function(actualError) {
                  actualErrorCode.push(actualError.errorcode);
                  actualMessageCode.push(actualError.messagecode);
                });
            ...
            Object.keys(actualOutputFields).forEach(function (fieldId) {
              ...
              expect(actualOutputFields[fieldId].errors).to.not.exist;
            });
          }

          // Check fields expected to be DISABLED
          var disabledFields = Object.keys(inputFields).filter(function (fieldId) {
            return inputFields[fieldId].value === 'DISABLED';
          });
          disabledFields.forEach(function (fieldId) {
            expect(actualOutputFields[fieldId].disabled).to.be.true;
          });
...
describe("Name Rules ", function () {
  var nameData = fs.readFileSync(__dirname + "/ruleDataName.csv");
  ruleValidation(nameData, this.title);
});

describe("Address Rules ", function () {
  var addressData = fs.readFileSync(__dirname + "/ruleDataAddress.csv");
  ruleValidation(addressData, this.title);
});

describe("Phone Email Rules ", function () {
  var phoneEmailData = fs.readFileSync(__dirname + "/ruleDataPhoneEmail.csv");
  ruleValidation(phoneEmailData, this.title);
});

describe("Bank Detail Rules ", function () {
  var addressData = fs.readFileSync(__dirname + "/ruleDataBankDetail.csv");
  ruleValidation(addressData, this.title);
});

describe("FBT Rules ", function () {
  var fbtData = fs.readFileSync(__dirname + "/ruleDataFbt.csv");
  ruleValidation(fbtData, this.title);
});
